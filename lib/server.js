...
function Server(opts) {
  ...
  this._bridge._hiddenAccessories = [];
  ...
  this._hiddenAccessories = [];
  this._overridenAccessories = [];
...
Server.prototype._loadAccessories = function() {
  ...
  for (var i=0; i<this._config.accessories.length; i++) {
    ...
    // Check for hidden accesories
    this._handleHiddenAccessory(accessoryConfig);

    // Check for accessory overrides
    this._handleAccessoryOverrides(accessoryConfig);
    ...
    if (this._isAccessoryHidden(accessory)) {
      // Hide overridden accessories from the bridge
      this._bridge._hiddenAccessories.push(accessory);
    } else {
      // otherwise, add it to the bridge
...
Server.prototype._handleHiddenAccessory = function(accessoryConfig) {
  if (accessoryConfig["hidden"]) {
    this._hiddenAccessories.push(accessoryConfig);
  }
}

Server.prototype._handleAccessoryOverrides = function(accessoryConfig) {
  var overrides = accessoryConfig["overrides"];
  if (overrides) {
    if (overrides["bridge"])
      throw new Error("Your config.json should not specify a bridge override.");
    // Pass bridge reference via config to allow overrides
    accessoryConfig["overrides"]["bridge"] = this._bridge;
    // Extract list of accessories that will be overridden and/or hidden by this accessory
    var overrideAccessories = overrides["accessories"];
    if (overrideAccessories && Array.isArray(overrideAccessories)) {
      overrideAccessories.forEach(function (overrideAccessory) {
        this._hiddenAccessories.push(overrideAccessory);
        this._overridenAccessories.push(overrideAccessory);
      }.bind(this));
    } else {
      throw new Error("Your config.json specifies an override, but does not specify any accessories to override.");
    }
  }
}

Server.prototype._isAccessoryHidden = function(accessory) {
  for (var i = 0; i < this._hiddenAccessories.length; i++) {
    if (this._hiddenAccessories[i].name == accessory.displayName) {
      return true;
    }
  }
  return false;
