...
Server.prototype._loadAccessories = function() {
  ...
  for (var i=0; i<this._config.accessories.length; i++) {
    ...
    if (this._isAccessoryOverridden(accessory)) {
...
Server.prototype._handleAccessoryOverrides = function(accessoryConfig) {
  ...
  if (overrides) {
    ...
    // Extract list of accessories that will be overridden by this accessory
...
Server.prototype._isAccessoryOverridden = function(accessory) {
  for (var i = 0; i < this._overridenAccessories.length; i++) {
    if (this._overridenAccessories[i].name == accessory.displayName) {
      return true;
    }
  }
  return false;
